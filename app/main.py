...
async def location(message: Message, state: FSMContext) -> None:
    ...
    if len(count_airports) > 1:
        ...
        await message.answer(
            answers.location.format(
                in_city=in_city, country=country, airport=airport_name
            ),
            reply_markup=KeyboardBuilder.main_reply_keyboard(),
        )
        await state.clear()
...
async def city_input(message: Message, state: FSMContext) -> None:
    ...
    cities = await asyncio.create_task(
        DatabaseQueries.find_airports_by_city_name(message.text)
    )
    if not cities:
        ...
        await message.answer(
            ...
            reply_markup=KeyboardBuilder.location_two_cities_keyboard(
                city_one, city_two
            ),
        )
        ...
        if len(count_airports) > 1:
            ...
            await message.answer(
                answers.cities_found.format(
                    city_name=city_name, airports_names=airports_text
                ),
                reply_markup=KeyboardBuilder.main_reply_keyboard(),
            )
            await state.clear()
            ...
            await message.answer(
                answers.city_found.format(
                    city_name=city_name, airport_name=airport_name
                ),
                reply_markup=KeyboardBuilder.main_reply_keyboard(),
            )
            await state.clear()
...
async def airport_not_found(message: Message) -> None:
    await message.answer(
        answers.city_or_location, reply_markup=KeyboardBuilder.location_reply_keyboard()
    )
...
async def choose_limit(message: Message, state: FSMContext) -> Optional[Message]:
    ...
    url = AviasalesAPI.create_default_request_url(
        user_data["destination"], user_data["limit"]
    )
    task_one_city = asyncio.create_task(
        AviasalesAPI.get_one_city_price(request_url=url)
    )
...
async def show_subscriptions(message: Message):
    ...
    subscriptions = await asyncio.create_task(
        DatabaseQueries.user_subscriptions(user_id)
    )

    if subscriptions:
        await message.answer(answers.subscriptions)
        for subscription in subscriptions:
            origin = await asyncio.create_task(
                DatabaseQueries.city_by_code(subscription[0])
            )
            arrival = await asyncio.create_task(
                DatabaseQueries.city_by_code(subscription[1])
            )
            answer_text = answers.subscription.format(
                origin=origin[0], arrival=arrival[0]
            )
            data = f"unsubscribe {subscription[0]} {subscription[1]}"
            await message.answer(
                answer_text, reply_markup=KeyboardBuilder.delete_subscription(data)
            )
    else:
...
async def five_cheapest_handler(message: Message) -> None:
    users_city = await asyncio.create_task(
    ...
    task_get_five = asyncio.create_task(
        AviasalesAPI.get_one_city_price(request_url=request_url)
    )
    ...
    for destination in result:
        ...
        destination_city = await asyncio.create_task(
            DatabaseQueries.city_by_code(destination["destination"])
        )
...
async def season_handler(message: Message) -> None:
    ...
    origin = users_city[3]
    cities_where_the_season = await asyncio.create_task(
        DatabaseQueries.cities_where_the_season()
    )
    random_cities_where_season = random.sample(
        cities_where_the_season, len(cities_where_the_season)
    )
    ...
    for ticket in ticket_list:
        ...
        weather_city = asyncio.create_task(
            WeatherApi.get_weather_with_coor(ticket["lat"], ticket["lon"])
        )
        result = await weather_city
        parsed_result = WeatherApi.small_parse_response(result)
        reply_keyboard = KeyboardBuilder.ticket_reply_keyboard(
            ticket["ticket_url"], data
        )
